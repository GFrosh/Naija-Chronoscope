...
Naija Chronoscope is a CMS / content website for a journalist (or group of journalists). It serves a static client site and an admin area for creating and managing articles, backed by a SQLite database and a small Express server.
...
Start with (1) health-check + smoke test and (2) server-side validation — both are small, low-risk, and improve reliability. After that, add a CI job that runs tests on push.
